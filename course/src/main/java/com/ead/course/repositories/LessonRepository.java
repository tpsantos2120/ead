...
public interface LessonRepository extends JpaRepository<LessonModel, UUID>, JpaSpecificationExecutor<LessonModel> {
    ...
    List<LessonModel> findAllLessonsByModuleId(@Param("moduleId") UUID moduleId);

    @Query(value = "SELECT * FROM tb_lessons WHERE module_id = :moduleId AND id = :lessonId", nativeQuery = true)
    Optional<LessonModel> findLessonByModuleId(@Param("moduleId") UUID moduleId, @Param("lessonId") UUID lessonId);
}
